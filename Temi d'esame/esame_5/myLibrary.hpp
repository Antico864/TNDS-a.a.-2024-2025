...
// Per rimuovere la cartella di cache di VScode:
//     rm -rf "~/Library/Application\ Support/Code/Cache/*"

// Per rimuovere i dati di cache di VScode:
//     rm -rf "~/Library/Application\ Support/Code/CachedData/*"

// Per convertire string in char*:
//     string nome_s = "Antico";
//     const char* nome_c = nome_s.c_str();

// Per convertire char* in string:
//     const char* nome_c = "Antico";
//     string nome_s ( nome-c );
...
template <typename T> double DevStd(vector<T> a) {
  ...
  double devst = sqrt(delta2sum / (counter - 1));
  return devst;
};
  ...
  inline virtual double Integrate(unsigned int nstep, const FunzioneBase &f) {
    ...
    if (nstep <= 0 || nstep % 2 != 0) {
      __throw_runtime_error(
          "Error: number of steps must be positive and even. ");
    }
...
void DoPosDistr(TH1F &hist, double h, double sigma,
...
double DevStd(double h, double sigma_v, BasicDifferentialEquation &res,
...
void FillTrajectoryGraph(BasicVectorialFunction &diff, vector<double> &state,
...
void FillRadiusGraph(BasicVectorialFunction &diff, vector<double> &state,
                     Runge_Kutta &rk, double T, double h, TGraph &graph) {
  unsigned int counter = 0;
  double r = 0.;
  for (double t = 0.; t <= 10 * T; t += h) {
...
ExpResults RunExperiment(unsigned int N, unsigned int seed, double sigmarelV0,
...
void FillWithError(vector<double>& sigV, vector<double>& sigC, const double err) {
