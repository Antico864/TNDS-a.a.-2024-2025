...
inline vector<T> operator*(const vector<T> &a, const T k) {
  ...
  for (unsigned int i = 0; i < static_cast<int>(a.size()); i++) {
    if (std::isnan(k) || std::isinf(k)) {
        cerr << "Errore: valore di k non valido: " << k << endl;
        exit(-1);
    }
    prod[i] = k * a[i];
  }
...
class diffeq3 : public BasicVectorialFunction {

  public:
      diffeq3(double alpha) { m_alpha = alpha; };

      inline vector<double> Eval(double t, const vector<double> &x) const override{
          // cout << "x = { " << x[0] << ", " << x[1] << ", " << x[2] << ", " << x[3] << " }" << endl;
          m_B = pow(x[0] * x[0] + x[1] * x[1], -0.5 * m_alpha);
          vector<double> v{x[2], x[3], -m_B*x[3], m_B*x[2]};
          return v;
      };
...
class diffeq3_3 : public BasicVectorialFunction {

  public:
      diffeq3_3(double alpha) { m_alpha = alpha; };

      inline vector<double> Eval(double t, const vector<double> &x) const override{
          // cout << "x = { " << x[0] << ", " << x[1] << ", " << x[2] << ", " << x[3] << " }" << endl;
          m_B = pow(x[0] * x[0] + x[1] * x[1], -0.5 * m_alpha) + 1.;
          vector<double> v{x[2], x[3], -m_B*x[3], m_B*x[2]};
          return v;
      };
...
class Runge_Kutta : public BasicDifferentialEquation {
    ...
    public:
      ...
      //  Funziona per vettori di stato di taglia qualsiasi.
      inline vector<double> Step(double t, const vector<double> x, double h, const BasicVectorialFunction &f) const override 
                {
                // if (sign(h) != 1) {
                //     cerr << "Integration step must be positive. " << endl;
                //     exit(-1);
...
void DoPosDistr(TH1F& hist, double h, double sigma, BasicDifferentialEquation& res, BasicVectorialFunction& f) {
    RandomGen ran(1);
    const double x0 = 0.;
    const double v0 = 1.;
...
double DevStd(double h, double sigma_v, BasicDifferentialEquation& res, BasicVectorialFunction& f) {
    RandomGen ran(1);
    const double x0 = 0.;
    const double v0 = 1.;
...
// Esame 3:
// Equazione differenziale del problema:



// 
void FillTrajectoryGraph(BasicVectorialFunction& diff, vector<double>& state, Runge_Kutta& rk, double T, double h, TGraph& graph) {
    unsigned int counter = 0;
...
double radius (double x, double y) {return (x * x + y * y); }

void FillRadiusGraph(BasicVectorialFunction& diff, vector<double>& state, Runge_Kutta& rk, double T, double h, TGraph& graph) {
  unsigned int counter = 0;
  double r = 0.;
  for(double t = 0.; t <= 10*T; t += h) {
...
void draw(TGraph& graph, TCanvas& c) {
    c.cd();
