...
class DOscillator : public BasicVectorialFunction {
    ...
    public:
      ...
      DOscillator(double omega0, double alpha) {
        if (omega0 < alpha) {
          throw runtime_error(
...
class Runge_Kutta : public BasicDifferentialEquation {
    ...
    public:
      ...
      virtual vector<double> Step(double t, const vector<double> x, double h, const BasicVectorialFunction &f) 
        ...
        double rkError(double tf, const vector<double> x, double h, const BasicVectorialFunction& f) {
            vector<double> xh = x;
            vector<double> xh2 = x;
            double t = 0.;
            ...
            while (t <= tf - h/2) {
                xh2 = Runge_Kutta::Step(t, xh2, h/2, f);
                if (counter2 % 2 == 0) {
                    xh = Runge_Kutta::Step(t, xh, h, f);
                    counter1++;
                }
                t += h/2;
                counter2++;
            }
            // counter2 <= 2*counter1 sempre!
            if (counter2%counter1 != 0) // Vuol dire che counter2 < 2*counter1
            // e quindi faccio evolvere di h/2 il sistema giusto
            {
                xh2 = Runge_Kutta::Step(t, xh2, h/2, f);
                counter2++;
            }
            // Attenzione a far evolvere fino allo stesso punto!
            // Costante dell'errore:
            m_k = 16. * fabs(xh[0] - xh2[0]) / (15. * pow(h, 4.));
            // cout << "counter1 = " << counter1 << endl;
            // cout << "counter2 = " << counter2 << endl;
            m_errorh = m_k * pow(h, 4.);
            return m_errorh;
        }
        double maxh(const double epsilon) {
            return pow(epsilon/m_k, 0.25);
        }
    private:
        double m_errorh;
        double m_k;
